 ...
 * calling `tick` from the store's `reduce` and emitting the effect for each notice is #53's
 * (`engine/game.ts`'s `tick`). `sync` (the manual **Sync** button) still waits on #14/#16 for a
 * `reduce` case of its own. */
